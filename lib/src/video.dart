...
import 'package:chitmaymay_player/src/model/models.dart';
import 'package:chitmaymay_player/src/utils/utils.dart';
import 'package:chitmaymay_player/src/widgets/video_loading.dart';
import 'package:chitmaymay_player/src/widgets/video_quality_picker.dart';
import 'package:chitmaymay_player/src/widgets/video_quality_widget.dart';
import 'package:chitmaymay_player/src/widgets/widget_bottombar.dart';
import 'package:video_player/video_player.dart';
import 'package:wakelock/wakelock.dart';

import 'responses/regex_response.dart';

class ChitMayMayPlayer extends StatefulWidget {
  /// **Video source**
  /// ```dart
  ...
  @override
  State<ChitMayMayPlayer> createState() => _ChitMayMayPlayerState();
}

class _ChitMayMayPlayerState extends State<ChitMayMayPlayer>
  ...
  final List iconsTitle = const [
    "Playback speed",
    "Subtitles",
    "Quality",
    "Audio"
  ];
  final List playbackOptions = [0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0];
  late int currentSpeed = 3;
  final ValueNotifier<double> _playbackSpeed = ValueNotifier(1);
  late final ValueChanged<VideoPlayerValue>? onRewind;
  late final ValueChanged<VideoPlayerValue>? onFastForward;

  /// The callback function execute when user tapped the play button.
  late final void Function()? onPlayButtonTap;
  @override
  void initState() {
    super.initState();
    urlCheck(widget.url);
    ...
    return Visibility(
      ...
      child: Align(
        ...
        child: PlayerBottomBar(
          ...
          onFullScreenTap: () => ScreenUtils.toggleFullScreen(fullScreen),
      ...
      builder: (context) {
        return Container(
          ...
          decoration: const BoxDecoration(
            ...
            color: Color.fromARGB(98, 22, 22, 22),
          ),
  ...
  Future _changeQualityBottomSheet() {
    ...
    return showModalBottomSheet(
      ...
      builder: (context) {
        return Container(
          ...
          child: ListView.builder(
            itemCount: playbackOptions.length,
            itemBuilder: (context, index) {
              final option = playbackOptions[index];
              return InkWell(
                child: Padding(
                ...
                onTap: () {
                  _playbackSpeed.value = playbackOptions[index];
                  currentSpeed = index;
